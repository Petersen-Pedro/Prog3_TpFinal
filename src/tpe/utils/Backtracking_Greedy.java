...
public class Backtracking_Greedy {
    ...
    // CONSIDERANDO EN TODO MOMENTO QUE UNA TAREA ES INDIVISIBLE, ES DECIR, NO PUEDE SER ASIGNADA A VARIOS PROCESADORES
    ...
    private void back(LinkedList<Tarea> tareas, LinkedList<Procesador> procesadores, int tiempoActual, int tareasAsignadas, int tiempoMaxProcNoRefrigerado) {
        if (tareasAsignadas == 0) {
            ...
            for (int i = 0; i < procesadores.size(); i++) {
                ...
                if ( ( !procesador.getTareas().contains(tareaActual) ) && (procesador.getTareasCriticas() < 2) ) {
                	if(procesador.isEstaRefrigerado()) {
	                    ...
	                    tareasAsignadas++;
                		}else if( ( procesador.getTiempoDeEjecucionAcumulado() + tareaActual.getTiempoEjecucion()  ) <= tiempoMaxProcNoRefrigerado) {
		                    procesador.agregarTarea(tareaActual);
		                    int nuevoTiempoProcesador = procesador.getTiempoDeEjecucionAcumulado();
